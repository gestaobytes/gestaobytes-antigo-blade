...
class ProjetosController extends StandardController {
    ...
    public function view($id) {
        ...
        $progressoProjeto = $this->tarefas->where('PROJ_CODIGO', $id)->sum('TAR_PROGRESSO');
        $qtdTarefas = $tarefas->count();
        
        $progressoProjeto = $progressoProjeto / $qtdTarefas;
        $inicio = $this->tarefas->where('PROJ_CODIGO', $id)->min('TAR_DTINICIO');
        $fim = $this->tarefas->where('PROJ_CODIGO', $id)->max('TAR_DTPRAZOESTIMADO');
...
//dd($comentarios);
        ...
        return view("{$this->nomeView}.view", compact('data', 'tarefas', 'comentarios', 'arquivos', 'bugs', 'etapas'))
                        ->with('progressoProjeto', $progressoProjeto)
                        ->with('inicio', $inicio)
                        ->with('fim', $fim)
                        ->with('codProjeto', $id)
